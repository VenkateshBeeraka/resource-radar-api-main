...
Resource Radar is primarily focuses on project management, encompassing tasks such as adding employees, creating projects, and allocating resources efficiently
...
# Dockerizing Resource radar project
...
Step2:  
create a docker network to communicate Resource radar and postgres database

```bash
docker network create resource-postgres-net
```
To verify
...
Step3:
Run the postgres container in the network in created network in detach mode
```bash
docker run --name postgresdb --network resource-postgres-net -e POSTGRES_PASSWORD=fission -e POSTGRES_DB=resource-radar -e POSTGRES_USER=fission -e POSTGRES_PASSWORD=fission -d postgres:latest
```
To verify
```bash
docker ps
...
Step4:
Run postgres in interactive mode
...
```bash
postgres -usa -pfission
```


Step5:
update application.yml file of our application according to docker postgres configuration and provide username, password and url according to docker postgres configuration
...
spring:
  datasource:
    password: fission
    url: jdbc:postgresql://localhost:5433/resource-radar
    username: fission
  jpa:
    hibernate:
      ddl-auto: update
...
```bash
docker run --network resource-postgres-net --name resource-radar-container -p 9090:9090 -d resource-radar
```
